...
{
    public partial class AutoProcForm_6051 : Form
    {
        ...
        private void AutoProcForm_6051_Load(object sender, EventArgs e)
        {
            //LotsTotrack.AddRange(GreenLotListFromTEDC); //testing
            try
            {
                AutoProcWebBrowser.Navigate(DispoSummaryUrl);
                LotsTotrack.AddRange(GreenLotListFromTEDC);
                ...
                foreach (DataGridViewRow datarow in AutoDispodataGridView.Rows)
                {
                    ...
                    if (datarow.Cells[1].Value.ToString().Trim() == "6053") //operation check
                    {
                    }
                    else // not at right operation
                    {
                        datarow.Cells[2].Value = "Not at 6053.";
                        lotgoodtogo = false;
                    }
                    ...
                    try
                    {
                        if (datarow.Cells[4].Value.ToString().Trim() == "Y") //instruction flag present
                        {
        ...
        private void inputlotids()
        {
            ...
            foreach (string lotID in LotsToAutoproc)
            {
                ...
                {
                    if (LotToTriggerAutoProcOn != lotID)
                    {
        ...
        private void SeeWhatLoadedAndUpdateStatus()
        {
            ...
            if (Wafer_level_webinfo.Contains("No data returned.")) //good to go, no dispo is needed
            {
                ...
                if (lot_level_webinfo.Contains("|REASON=HOLD for DISPOSITION per XML Tag|RESULT=AUTO_HOLD|USERNAME=|OPERATION\">6051</DIV>")) //6053 lot level hold is there, clear it
                {
                    UpdateStatusOnLot(lotid, "6053 lot level hold found", Color.LightGreen);
                    LotPassedDispoChecks = lotid;
                }
                else if (!(lot_level_webinfo.Contains("<OPTION selected value=AUTO_HOLD>AUTO_HOLD</OPTION>")))
                {
                    UpdateStatusOnLot(lotid, "No 6053 lot level hold found", Color.LightGreen);
                    LotToTriggerAutoProcOn = lotid;
                    LotPassedDispoChecks = lotid;
                }
                else
                {
        ...
        private void PassAndSaveLotLevelDispo()
        {
            ...
            try
            {
                ...
                foreach (HtmlElement el in elc)
                {
                    ...
                    {
                        if (el.Id.Contains("OPERATION=6051"))
                        {
        ...
        private void TestButton_Click(object sender, EventArgs e)
        {
            ...
            lotsthatweretriggered.AddRange(LotsToAutoproc);
            CheckLotStatusForm(lotsthatweretriggered);
        }
