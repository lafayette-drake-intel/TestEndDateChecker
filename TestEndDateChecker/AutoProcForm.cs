...
{
    public partial class AutoProcForm : Form
    {
        ...
        private void AutoProcForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (DataGridViewRow datarow in AutoDispodataGridView.Rows)
                {
                    ...
                    if (datarow.Cells[1].Value.ToString().Trim() == "6753") //operation check
                    {
                    }
                    else // not at right operation
                    {
                        datarow.Cells[2].Value = "Not at 6753.";
                        lotgoodtogo = false;
                    }
        ...
        private void SeeWhatLoadedAndUpdateStatus()
        {
            ...
            if (Wafer_level_webinfo.Contains("No data returned.")) //good to go, no dispo is needed
            {
                ...
                if (lot_level_webinfo.Contains("|REASON=HOLD for DISPOSITION per XML Tag|RESULT=AUTO_HOLD|USERNAME=|OPERATION\">6751</DIV>")) //6751 lot level hold is there, clear it
                {
                    UpdateStatusOnLot(lotid, "6751 lot level hold found", Color.LightGreen);
                    LotPassedDispoChecks = lotid;
                }
                else if (!(lot_level_webinfo.Contains("<OPTION selected value=AUTO_HOLD>AUTO_HOLD</OPTION>")))
                {
                    UpdateStatusOnLot(lotid, "No AUTO_HOLD 6751 lot level hold found", Color.LightGreen);
                    LotPassedDispoChecks = lotid;
                }
                else
                {
        ...
        private void PassAndSaveLotLevelDispo()
        {
            ...
            try
            {
                ...
                foreach (HtmlElement el in elc)
                {
                    ...
                    {
                        if (el.Id.Contains("OPERATION=6751"))
                        {
